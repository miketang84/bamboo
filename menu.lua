...
Menu = Node:extend {
	...
	__desc = 'Menu';
	__fields = {
		{ 'name', 'text', false },						-- 菜单节点的内部名称
		{ 'rank', 'text', true },						-- 菜单节点在整个菜单树中的级别，为数字
		{ 'title', 'text', true },						-- 菜单节点标题
		{ 'prompt', 'text', true },						-- 菜单节点提示（当鼠标放上去的时候，非必须）
		{ 'content', 'textarea', true },				-- 菜单所链到的页面对象ID，或直接是URL地址，一个菜单节点只能链到一个页面ID上去

		{ 'parent', 'text', true },						-- 菜单节点的父菜单节点id，如果为空，则表明本节点为顶级节点
		{ 'children', 'text', true },					-- 此菜单节点的子菜单节点id列表字符串，默认每个菜单节点下面都可以接子节点
